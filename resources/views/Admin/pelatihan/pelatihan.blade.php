                    ...
                    <tr
                      ...
                      <th class="px-4 py-3">
                        <div class="flex items-center">
                          @php
                            $sortUrl = URL::current().'?'.http_build_query(array_merge(request()->except('sort', 'direction'), ['sort' => 'bidang', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']));
                          @endphp
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                      <td class="px-4 py-3">
                        <div class="flex items-center text-xs">
                          ...
                          <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                            <img class="object-cover w-full h-full rounded-full" src="{{ $d->users->avatar }}" alt="" 
                              loading="lazy" />
                            <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                          </div>
                          <div>
                            <p class="font-semibold">{{ $d->users->name }}</p>
                      ...
                      <td class="px-4 py-3 text-xs">
                        <span
                          class=" px-2 py-1 font-semibold leading-tight 
                            {{ 
                              $d->status == 'Belum Tervalidasi' ? 'text-red-700 bg-red-100 rounded-full dark:text-red-100 dark:bg-red-700' :
                              ($d->status == 'Tervalidasi' ? 'text-green-700 bg-green-100 rounded-full dark:text-green-100 dark:bg-green-700' : '')
                            }}">
                          {{ $d->status }}
                        </span>
                      </td>
                      <td class="px-4 py-3 text-xs">
                        <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
                          href="{{ route('pelatihan.detail',['id' => $d->id]) }}">
                          <span
